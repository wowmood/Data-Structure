...
void insertFirstNode(linkedList_h* L, char* x) {
	...
	strcpy(newNode->data, x);						// 새 노드의 데이터 필드에 x 저징
	newNode->link = L->head;
	L->head = newNode;
...
void insertMiddleNode(linkedList_h* L, listNode* pre, char* x) {
	...
	if (L->head == NULL) {		// 공백 리스트인 경우
		...
		newNode->link = pre->link;		// 포인터 pre 의 노드 뒤에 새 노드 연결
		pre->link = newNode;
	}
}
...
void deleteNode(linkedList_h* L, listNode* p) {
	listNode* pre;
	if (L->head == NULL) return;	// 공백 리스트면 리턴
	if (L->head->link == NULL) {	// 리스트에 노드가 한 개만 있는 경우
		...
		pre = L->head;				// 삭제할 노드 p의 선행자 노드를 포인터 pre를 이용해 찾음
		while (pre->link != p) {	// pre의 링크가 p가 아닐 동안
			pre = pre->link;		// 다음 링크로 이동
		}							// pre 링크가 p의 값과 같으면 (p의 선행자 노드 찾음)
...
listNode* searchNode(linkedList_h* L, char* x) {
	listNode* temp;
	temp = L->head;		// 리스트 시작 포인터가 가리키는 값(= 첫 번째 노드 주소)을 temp에 넣음
	while (temp != NULL) {		// 리스트의 끝까지
		if (strcmp(temp->data, x) == 0) return temp;	// 노드의 값이 x와 일치하면 temp = 현재 노드의 값을 return
		else temp = temp->link;		// 찾는 값이 아니면 다음 노드로 이동
	}
	return temp;		// 값을 찾지 못하면 temp == NULL 값을 반환
}